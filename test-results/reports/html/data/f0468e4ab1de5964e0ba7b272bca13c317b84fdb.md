...
Error: strict mode violation: getByText('RAIA') resolved to 59 elements:
    1) <span class="font-extrabold text-sm tracking-tight">RAIA</span> aka getByRole('link', { name: 'RAIA 0.1.0' })
    2) <span class="text-xs font-semibold">Buscar en RAIA...</span> aka getByText('Buscar en RAIA...').first()
    3) <span class="text-xs font-semibold">Buscar en RAIA...</span> aka getByRole('banner').getByRole('button', { name: 'Buscar en el portal (Presione' })
    4) <span class="px-2.5 py-0.5 rounded-full text-[9px] font-bold bg-blue-950 text-blue-300 border border-blue-800 uppercase tracking-wider">RAIA Release 0.1.0</span> aka getByText('RAIA Release 0.1.0')
    5) <h1 class="text-xl md:text-2xl font-extrabold tracking-tight bg-gradient-to-r from-white via-slate-100 to-slate-300 bg-clip-text text-transparent">RAIA Architecture Overview Portal</h1> aka getByRole('heading', { name: 'RAIA Architecture Overview' })
    6) <span class="text-xs font-semibold">Buscar en RAIA...</span> aka locator('#main-content').getByRole('button', { name: 'Buscar en el portal (Presione' })
    7) <span class="text-[8px] font-mono font-bold text-slate-400 tracking-wider uppercase block mt-0.5">RAIA-MOD-001</span> aka getByRole('link', { name: 'v0.1.0 Information' })
    8) <span class="font-bold text-slate-700">RAIA Maintainers</span> aka getByRole('link', { name: 'v0.1.0 Information' })
    9) <span class="font-semibold text-slate-600 truncate">RAIA-MOD-003, RAIA-MOD-012</span> aka getByRole('link', { name: 'v0.1.0 Information' })
    10) <span class="text-[8px] font-mono font-bold text-slate-400 tracking-wider uppercase block mt-0.5">RAIA-MOD-002</span> aka getByRole('link', { name: 'v0.1.0 Control Record Model' })
    ...
