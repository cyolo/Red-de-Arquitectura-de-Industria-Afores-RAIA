...
import { getReferenceModelGaps } from "../../../domain/repositories/referenceModelRepository";
import { AlertCircle } from "lucide-react";

export default function ReferenceModelGapPanel() {
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6" data-testid="gaps-tab-content">
      <div className="border-b border-slate-200 pb-3 shrink-0">
        <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
          Gaps de Composición de la Industria
        </h3>
      </div>
        ...
        {gaps.map((gap) => (
          <div
            ...
            className={`border rounded-xl p-4 flex items-start gap-3.5 shadow-xs ${
              gap.severity === "P1" ? "bg-amber-50/40 border-amber-200" : "bg-slate-50 border-slate-200"
            }`}
          >
            <div className={`p-2 rounded-lg shrink-0 ${
              gap.severity === "P1" ? "bg-amber-100 text-amber-700" : "bg-slate-200 text-slate-700"
            }`}>
              <AlertCircle size={16} />
            </div>
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={`text-[8px] font-extrabold px-1.5 py-0.2 rounded border uppercase ${
                  gap.severity === "P1" ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-slate-105 text-slate-700 border-slate-250"
                }`}>
