...
try {
  ...
  console.warn("Could not read landscape file. Using empty fallback list.");
}
...
function runMigration(write = false) {
  ...
  const domains = capabilityDomains.map((d) => {
  ...
  capabilityL2List.forEach((c) => {
    ...
    capabilities.push({
      ...
      capabilityType: domain ? domain.capabilityType : "industry-core",
      valueContribution: "mandatory",
  ...
  const assessments = capabilities.map((cap) => {
    ...
    return {
      ...
      capabilityId: cap.id,
      assessmentScope: "industry-reference",
      currentMaturity: "defined",
      strategicImportance: "high",
      performance: "meets-expectation",
      health: "healthy",
      investmentPriority: "improve",
      evidenceIds: [],
      kpiIds: [],
  ...
  const gaps = [
    {
      id: "RAIA-CAPGAP-0001",
      gapType: "capability-without-realization",
...
const writeOption = process.argv.includes('--write') || true;
